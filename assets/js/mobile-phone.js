(function () {
  ...
  function initMobileMenuState() {
    if (!isPhone()) return;
    var reveal = document.querySelector('.meanmenu-reveal');
    if (!reveal) return;

    function syncState() {
      var open = reveal.classList.contains('meanclose');
      document.body.classList.toggle('mobile-nav-open', open);
      reveal.setAttribute('aria-expanded', String(open));
      reveal.setAttribute('aria-label', open ? 'Close navigation' : 'Open navigation');
    }
  ...
  function initMobileProjectMap() {
    ...
    controls.innerHTML = [
      '<button type="button" data-map-action="rotate" aria-label="Best landscape view" title="Best landscape view"><i class="bi bi-phone-landscape" aria-hidden="true"></i></button>',
      '<button type="button" data-map-action="reset" aria-label="Reset map view" title="Reset map view"><span aria-hidden="true">1:1</span></button>',
    ...
    function clearLandscapeFallback() {
      stage.classList.remove('is-landscape-fallback');
      document.body.classList.remove('map-landscape-fallback');
    }

    function applyLandscapeFallback() {
      stage.classList.add('is-landscape-fallback');
      document.body.classList.add('map-landscape-fallback');
    }

    function updateRotateButton(button) {
      var active = isFullscreen() || stage.classList.contains('is-landscape-fallback');
      button.setAttribute('aria-label', active ? 'Exit landscape map view' : 'Best landscape view');
      button.setAttribute('title', active ? 'Exit landscape map view' : 'Best landscape view');
      button.innerHTML = active ? '<i class="bi bi-fullscreen-exit" aria-hidden="true"></i>' : '<i class="bi bi-phone-landscape" aria-hidden="true"></i>';
    }

    async function toggleLandscapeView(button) {
      if (isFullscreen() || stage.classList.contains('is-landscape-fallback')) {
        clearLandscapeFallback();
        if (screen.orientation && screen.orientation.unlock) {
          try { screen.orientation.unlock(); } catch (error) { /* no-op */ }
        }
        if (isFullscreen()) {
          try { await exitFullscreen(); } catch (error) { /* no-op */ }
        }
        updateRotateButton(button);
        window.setTimeout(renderTransform, 50);
        return;
      }

      var fullscreenWorked = false;
      try {
      ...
      if (fullscreenWorked && screen.orientation && screen.orientation.lock) {
        try {
      ...
      if (fullscreenWorked && !landscapeLocked) {
        try { await exitFullscreen(); } catch (error) { /* no-op */ }
        fullscreenWorked = false;
      }
    ...
    document.addEventListener('fullscreenchange', function () {
      ...
      if (rotateButton) updateRotateButton(rotateButton);
